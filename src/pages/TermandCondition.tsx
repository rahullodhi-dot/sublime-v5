...
const TermandCondition = () => {
    ...
    const breadcrumbItems = [
        { label: "Term and Condition", path: `TnC/`, isBold: true },
    ...
    // Function: index → Alphabet
    function getAlphabetByIndex(index: number) {
        return ALPHABETS[index] || "";
    }


    // Function: index → Roman
    function getRomanByIndex(index: number) {
        return ROMANS[index] || "";
    }
    ...
    const ALPHABET_SECTIONS = [
        ...
        `This document is published in accordance with the provisions of Rule 3 (1) of the Information Technology (Intermediaries guidelines) Rules, 2011 that require publishing the rules and regulations, privacy policy and Terms of Use for access or usage of domain name sublimehouseoftea.com ('Website'), including the related mobile site and mobile application (hereinafter referred to as 'Platform').`,

        `The Platform is owned by SUBLIME, a company incorporated under the Companies Act, 1956 with its registered office at No 19 Prestige Falcon Tower Brunton Road Bangalore 560025 (hereinafter referred to as ‘Platform Owner’, 'we', 'us', 'our').`,
    ...
    const divClass = `flex gap-2 mb-6 `

    return (
        <section>

            <TopSection breadCrumnb={breadcrumbItems} title="Term And Condition" />

            <div className='max-w-[1400px] text-lg leading-7 mx-auto px-6 py-20'>
                <div>
                    {
                        ALPHABET_SECTIONS.map((item, i) => (
                            <div className={divClass} >
                                <span className='text-[#9a7523]'>{getAlphabetByIndex(i)}</span>  <p> {item}</p>
                            </div>
                        ))
                    }
                </div>
                ...
                <div >
                    {
                        ROMAN_CLAUSES.map((item, i) => (
                            <div className={divClass}><span className='text-[#9a7523]'>{getRomanByIndex(i)}</span>   <p> {item}</p></div>
                        ))
                    }
                </div>
                ...
                <div className="mt-16">
                    ...
                    {FAQS.map((faq, i) => (
                        <div key={i} className=" py-4 ">
                            <div
                                className="flex justify-between  px-5 bg-[#D6C27A] rounded-lg  py-3 items-center cursor-pointer"
                                onClick={() => toggleFaq(i)}
                            >
                                <p style={{fontFamily:"gotham-book"}} className="font-medium px-2">{faq.q}</p>
                                {openFaq === i ?<div className='p-2  border-black border-2 rounded-full'> <Minus size={18} /></div>: <div className='p-2 border-black border-2 rounded-full'> <Plus size={18} /></div>}
                            </div>

                            <div
                            
                                className={`overflow-hidden px-6  transition-all duration-500 ease-in-out ${openFaq === i ? "max-h-40 mt-3" : "max-h-0"
                                    }`}
                            >
                                <p style={{fontFamily:"gotham-book"}} className=" text-sm leading-7  lowercase text-gray-600">{faq.a}</p>
                            </div>
                        </div>
                    ))}
                </div>
